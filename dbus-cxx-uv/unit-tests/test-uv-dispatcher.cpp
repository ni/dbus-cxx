...
int main(int argc, char** argv){
    ...
    if( dispatch_type == "standalone" ){
        //use_uv_dispatcher = false;
        fprintf(stderr, "libuv doesn't support threaded/standalone\n");
        return -1;
    }

    if( use_uv_dispatcher ){
        disp = DBus::Uv::UvDispatcher::create();
    }else{
